...
for(int b =1 ; b<=a; b++) {
	System.out.println("enter hours for customer"+ b);

	totalCharges = totalCharges + calculateCharges();
}
double yesterdayCharges = totalCharges;
System.out.println("\ntotal charges for all customers for yesterday : "+totalCharges);

System.out.println("\nenter number of hours for each customer for today");

while(input.hasNext()) {
	
	System.out.println("enter for customer"+ i+ "  or ctrl Z to end number of customers entered");
...
System.out.println("\ntotal charges for all customers so far: "+totalCharges);
}
